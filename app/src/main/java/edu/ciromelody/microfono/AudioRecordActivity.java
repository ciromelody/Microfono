...
public class AudioRecordActivity extends AppCompatActivity {
    ...
    final private int REQUEST_WRITE_EXTERNAL_STORAGE = 124;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        textViewDebug = findViewById(R.id.textView);

        textViewDebug.setText("Debug");
        chiediIPermessiDiScrittura(textViewDebug);

        findViewById(R.id.btnStart).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (ContextCompat.checkSelfPermission(AudioRecordActivity.this, android.Manifest.permission.RECORD_AUDIO)
    ...
    private void chiediIPermessiDiScrittura(TextView textViewDebug) {
        ...
        switch (requestCode) {
            ...
            case REQUEST_READ_EXTERNAL_STORAGE:
                for (int i = 0; i < permissions.length; i++) {
                    ...
                    if (permission.equals(android.Manifest.permission.READ_EXTERNAL_STORAGE) || permission.equals(android.Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
                        if (grantResult == PackageManager.PERMISSION_GRANTED) {
                            ReadExternalStorage();
                            Toast.makeText(AudioRecordActivity.this, "Permission Log File ottenuti vedi in MEMORIA TELEFONO", Toast.LENGTH_SHORT).show();
                        } else {
                            Toast.makeText(AudioRecordActivity.this, "Permission Log File Denied", Toast.LENGTH_SHORT).show();
                        }
                    }
                }
        ...
        protected Object[] doInBackground(File... files) {
            ...
            try {
                ...
                if (ActivityCompat.checkSelfPermission(ctx, Manifest.permission.RECORD_AUDIO) != PackageManager.PERMISSION_GRANTED) {
                    ...
                    return new Object[] { files[0].length(), endTime - startTime };//Cioè non ritorna niente perchè non ci sono permessi
                }
    ...
    private File generaFile() {
        // preferisco questa directory perchè è accessibile da telefono ma oltre a questi permessi ocorre
        //AUTORIZZARE MANUALMENTE ALLA SCRITTURA DI TUTTI I FILE
        File directory = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator +"recordaudio");
       // File directory = new File(getApplicationContext().getExternalFilesDir(null).getAbsolutePath(), "recordAudio") ;
        if(!directory.exists())
        {
            directory.mkdir();
            if(!directory.exists())
    ...
    public void ReadExternalStorage() {
        ...
        textViewDebug.setText(state);
        File directory = new File(getApplicationContext().getExternalFilesDir(null).getAbsolutePath(), "recordmicrofono") ;
        if(!directory.exists())
        {
            directory.mkdir();
            if(!directory.exists())
        ...
        try {
            ...
            writeTextFile(newFile, "This is a test by me. Roger:"+newFile.getCanonicalPath());
            String text = readTextFile(newFile);
            textViewDebug.setText(text);
            if (!newFile.delete()) {
                textViewDebug.setText("Couldn't remove temporary file");
            }
        } catch (IOException e) {
            textViewDebug.setText("Something went wrong! " + e.getMessage());
        }
    }
