package edu.ciromelody.microfono.registraaudio;
...
public class RecordWaveTask extends AsyncTask<File, Void, Object[]> {
    ...
    private Context ctx;
    private static RecordWaveTask recordTask = null;
    ...
    public static  RecordWaveTask  getInstance(Context ctx) {

        // Check if the instance is already created
        if(recordTask == null) {
            // synchronize the block to ensure only one thread can execute at a time
            synchronized (RecordWaveTask .class) {
                // check again if the instance is already created
                if(recordTask == null) {

                    // create the singleton instance
                    recordTask = new RecordWaveTask (ctx);
                }
            }
        }
    ...
    public  void stopTask(){
        ...
        if (!recordTask.isCancelled() && recordTask.getStatus() == AsyncTask.Status.RUNNING) {
            ...
            Toast.makeText(ctx, "Task not running.", Toast.LENGTH_SHORT).show();
        }
    }
